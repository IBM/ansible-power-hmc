...
def validate_proc_mem(system_dom, proc, mem, max_proc, min_proc, max_mem, min_mem, weight, min_proc_unit, max_proc_unit, proc_unit=None):
    ...
    if proc_unit:
        if weight not in range(256):
            raise ProcMemValidationError("weight value should be in between 0 to 255")
        ...
        if Decimal(str(proc_unit)) % Decimal(min_proc_unit_per_virtproc) != Decimal('0.0'):
            raise ProcMemValidationError("Input processor units: {0} must be a multiple of {1}".format(proc_unit, min_proc_unit_per_virtproc))
    ...
    if mem % lmb > 0:
        raise ProcMemValidationError("Requested mem value not in mutiple of block size:{0}".format(curr_avail_lmb))

    if mem > int_avail_mem:
        raise ProcMemValidationError("Available system memory is not enough. Provide value on or below {0}".format(curr_avail_mem))
