...
class HmcCommandStack():
    ...
    HMC_CMD_OPT = {'LSHMC': {'-N': ' -n ',
                   ...
                   'CHHWRES': {'-R': {'VIRTUALIO': ' -r virtualio ', 'IO': ' -r io ', 'MEM': ' -r mem ', 'MEMPOOL': ' -r mempool ',
                                      'RSPOOL': ' -r rspool ', 'PROC': ' -r proc ', 'PROCPOOL': ' -r procpool ', 'HEA': ' -r hea ',
                                      'SRIOV': ' -r sriov ', 'ACCEL': ' -r accel ', 'PMEM': ' -r pmem '},
                               '--RSUBTYPE': {'SLOT': ' --rsubtype slot ', 'BUS': ' --rsubtype bus ', 'IOPOOL': '  --rsubtype iopool ',
                                              'TAGGEDIO': ' --rsubtype taggedio ', 'ETH': ' --rsubtype eth ', 'FC': '  --rsubtype fc ',
                                              'SCSI': ' --rsubtype scsi ', 'SERIAL': ' --rsubtype serial ', 'HSL': '  --rsubtype hsl ',
                                              'VIRTUALOPTI': ' --rsubtype virtualopti ', 'VNETWORK': ' --rsubtype vnetwork ',
                                              'VNIC': ' --rsubtype vnic ', 'VSWITCH': ' --rsubtype vswitch ', 'ADAPTER': ' --rsubtype adapter ',
                                              'LOGPORT': ' --rsubtype logport ', 'PHYSPORT': ' --rsubtype physport',
                                              'PGDEV': ' --rsubtype pgdev ', 'RSDEV': ' --rsubtype rsdev ',
                                              'VNICBKDEV': ' --rsubtype vnicbkdev ', 'GZIP': ' --rsubtype gzip ', 'VOLUME': ' --rsubtype volume '},
                               '--LEVEL': {'SYS': ' --level sys ', 'LPAR': ' --level lpar ', 'ETH': ' --level eth ',
                                           'ETHC': ' --level ethc ', 'ROCE': ' --level roce '},
                               '-M': ' -m ',
                               '-T': ' -t ',
                               '--TID': ' --tid ',
                               '--PROCS': ' --procs ',
                               '--PROCUNITS': ' --procunits ',
                               '--5250CPWPERCENT': ' --5250cpwpercent ',
                               '--ENTITLED': ' --entitled ',
                               '-W': ' -w ',
                               '-D': ' -d ',
                               '--FORCE': ' --force ',
                               ...
                               '-S': ' -s ',
                               '-Q': ' -q ',
                               '-O': {'A': ' -o a ', 'R': ' -o r ', 'S': ' -o s ', 'SO': ' -o so ', 'RS': ' -o rs ', 'C': ' -o c ', 'ACT': ' -o act',
                                      'E': ' -o e ', 'D': ' -o d ', 'F': ' -o f '},
                               '--ID': ' --id ',
                               '--PORT': ' --logport ',
                               '--DEVICE': ' --device ',
                               ...
                               '--VNETWORK': ' --vnetwork ',
                               '--VOLUME': ' --volume ',
                               '-A': {'BACKING_DEVICES': 'backing_devices', 'PORT_VLAN_ID': 'port_vlan_id', 'PVID_PRIORITY': 'pvid_priority',
                                      'ALLOWED_VLAN_IDS': 'allowed_vlan_ids', 'MAC_ADDR': 'mac_addr',
                                      'ALLOWED_OS_MAC_ADDRS': 'allowed_os_mac_addrs', 'AUTO_PRIORITY_FAILOVER': 'auto_priority_failover',
                                      'ADAPTER_ID': 'adapter_id', 'LOGICAL_PORT_ID': 'logical_port_id',
                                      'FAILOVER_PRIORITY': 'failover_priority', 'PAGING_VIOS_NAMES': 'paging_vios_names',
                                      'POOL_MEM': 'pool_mem', 'MAX_POOL_MEM': 'max_pool_mem', 'MEM_DEDUP': 'mem_dedup',
                                      ...
                                      'MAX_ROCE_LOGICAL_PORTS': 'max_roce_logical_ports', 'CURR_ROCE_LOGICAL_PORTS': 'curr_roce_logical_ports',
                                      'MAX_ROCE_PROMISC_PORTS': 'max_roce_promisc_ports', 'MAX_ROCE_DIAG_PORTS': 'max_roce_diag_ports',
                                      'MIN_ROCE_CAPACITY_GRANULARITY': 'min_roce_capacity_granularity',
                                      'SUPPORTED_MAX_ROCE_ALLOWED_VLAN_IDS': 'supported_max_roce_allowed_vlan_ids',
                                      'SUPPORTED_MAX_ROCE_ALLOWED_OS_MAC_ADDRS': 'supported_max_roce_allowed_ os_mac_addrs',
                                      'PHYS_PORT_SUB_LABEL': 'phys_port_sub_label', 'RECV_FLOW_CONTROL': 'recv_flow_control',
                                      'TRANS_FLOW_CONTROL': 'trans_flow_control', 'VEB_MODE': 'veb_mode', 'VEPA_MODE': 'vepa_mode',
                                      'LOGICAL_PORT_TYPE': 'logical_port_type', 'CONFIG_ID': 'config_id', 'CAPACITY': 'capacity',
                                      'MAX_CAPACITY': 'max_capacity', 'QOS_PRIORITY': 'qos_priority',
                                      'ADAPTER_TYPE': 'adapter_type', 'REMOTE_LPAR_ID': 'remote_lpar_id',
                                      'REMOTE_SLOT_NUM': 'remote_slot_num', 'IS_TAGGED': 'is_tagged', 'VLAN_ID': 'vlan_id', 'MIGRATABLE': 'migratable',
                                      'BACKUP_DEVICE_TYPE': 'backup_device_type', 'BACKUP_DEVICE_SLOT_NUM': 'backup_device_slot_num',
                                      'BACKUP_VNIC_BACKING_DEVICE': 'backup_vnic_backing_device', 'BACKUP_VETH_VNETWORK': 'backup_veth_vnetwork',
                                      'SIZE': 'size', 'AFFINITY': 'affinity', 'REQUESTED_NUM_SYS_HUGE_PAGES': 'requested_num_sys_huge_pages',
